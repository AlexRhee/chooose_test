...
class OrderImpExpXML_GeneralCaseImporter {
    ...
    public function wf_xml_process_order_general_new_insert($post)
    {
        ...
        $order_data = array(
            ...
            'post_name'     => 'order-' . date_format($post['postmeta']['OrderDate'], 'M-d-Y-hi-a'),
            'post_type'     => 'shop_order',
            'post_title'    => 'Order &ndash; ' . date_format($post['postmeta']['OrderDate'], 'F d, Y @ h:i A'), 
            ...
            'post_date'     => date_format($post['postmeta']['OrderDate'], 'Y-m-d H:i:s e'),
            'comment_status' => 'open'
        );
        ...
        else
        {
            ...
            add_post_meta($order_id, '_completed_date', date_format( $post['postmeta']['OrderDate'], 'Y-m-d H:i:s e'), true);
            add_post_meta($order_id, '_order_currency', $post['postmeta']['OrderCurrency'], true);
            add_post_meta($order_id, '_paid_date', date_format( $post['postmeta']['OrderDate'], 'Y-m-d H:i:s e'), true);
            ...
            update_post_meta($order_id, 'shipping_tax_total', $post['postmeta']['ShippingTaxTotal']);


            update_post_meta($order_id, 'tax_total', $post['postmeta']['TaxTotal']);
    ...
    public function wf_xml_process_order_general_order_exist($post,$import_decision)
    {
        ...
        if(!empty($post['postmeta']['OrderDate'])){
            if ($post['postmeta']['OrderDate'] instanceof DateTime) {
                    $formatted_date = date_format( $post['postmeta']['OrderDate'], 'Y-m-d H:i:s e');
            }
            update_post_meta($order_id, '_completed_date', $formatted_date);
            update_post_meta($order_id, '_paid_date', $formatted_date);
        }
