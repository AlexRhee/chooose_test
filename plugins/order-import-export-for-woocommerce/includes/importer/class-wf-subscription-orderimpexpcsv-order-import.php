...
// Remainder
    ...
    private function process_subscription_orders($data) {
        ...
        if (is_wp_error($user_id)) {
            ...
            if (!$merging && $is_order_exist) {
                ...
                $this->hf_log_data_change('hf-subscription-csv-import', sprintf(__('> &#8220;%s&#8221;' . $usr_msg, 'wf_order_import_export'), esc_html($data['subscription_id'])), true);
                unset($data);
                return;
            } else {
		
		if( !empty($data['post_parent']) )	//Check whether post_parent (Parent order ID) is an order or not
		{
		...
		if ($is_order_exist) {			//Execute this when subscription already exist
            ...
            $download_permission_detils = array();
            foreach ($data['post_meta'] as $meta_data) {
                ...
                if('_download_permissions' == $meta_data['key']){
                    $download_permission_detils['download_permissions'] = ($meta_data['value'])?$meta_data['value']:0;
                }
            }
		...
		return;
            ...
            if ($data['manualy_set']) {
		$is_manual = false;
                ( WC()->version >= '2.2.0' ) ? $subscription->set_requires_manual_renewal($is_manual) : $subscription->update_manual($is_manual);
                ...
                $result['warning'] = isset($result['warning']) ? $result['warning'] : array();
		$warning = array_merge($result['warning'], self::set_payment_meta($subscription, $data));
                $this->hf_log_data_change('hf-subscription-csv-import', @sprintf(__($warning, 'wf_order_import_export'), $data['subscription_id']), true);
                //log warning
            }
                ...
                if (is_numeric($data['order_items'])) {
                    ...
                    if (!empty($order_items)) {
                        foreach ($order_items as $order_item) {
                            ...
                            $result['items'] .= self::add_product($item_data, $subscription, $download_permission_detils, $chosen_tax_rate_id) . '<br/>';
	...
	elseif( !empty($order) )
	{
        ...
        $default_args = array(
            ...
            'created_via' => (!empty($order) ) ? ( ( WC()->version > '2.7.0' ) ? $order->get_created_via() : $order->created_via ) : '',
            'order_version' => (!empty($order) ) ? ( ( WC()->version >= '2.7.0' ) ? $order->get_version() :$order->order_version ) : WC_VERSION,
            'currency' => (!empty($order) ) ? ( ( WC()->version >= '2.7.0' ) ? $order->get_currency() : $order->order_currency ) : get_woocommerce_currency(),
            'prices_include_tax' => (!empty($order) ) ? ( ( ( WC()->version >= '2.7.0' ) ? $order->get_prices_include_tax() : $order->prices_include_tax ) ? 'yes' : 'no' ) : get_option('woocommerce_prices_include_tax'), // we don't use wc_prices_include_tax() here because WC doesn't use it in wc_create_order(), not 100% sure why it doesn't also check the taxes are enabled, but there could forseeably be a reason
        );
        ...
        if ($subscription_exist) {
            ...
            $subscription_data['import_id'] = $args['import_id'];
            $subscription_data['post_type'] = 'shop_subscription';
            $subscription_data['post_status'] = $subscription_exist['subscription_status'];
            $subscription_data['ping_status'] = 'closed';
    ...
    public static function set_payment_meta($subscription, $data) {
        ...
        $payment_method = ( WC()->version >= '2.7.0' ) ? $subscription->get_payment_method() : $subscription->payment_method;
    ...
    public static function add_product($details, $subscription, $data, $chosen_tax_rate_id) {
        ...
        if (!$_product) {
            ...
            $product_string = sprintf('<a href="%s">%s</a>', get_edit_post_link((WC()->version >= '2.7.0') ? $_product->get_id() : $_product->id), $line_item_name);
