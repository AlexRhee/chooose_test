...
//							// invalid item
//							$WF_CSV_Order_Import->log->add('hf-order-csv-import', sprintf( __( '> > Skipped. Missing SKU, quantity or total for %s on row %s.', 'wf_order_import_export' ),  'line_item_' . $i, $row ) );
//							$skipped++;
//                                                      $i++;
...
//						}
						...
						$i++;
                                ...
                                // standard format
                                $coupon_item = array();
				if ( ! empty( $item['coupon_items'] ) ) {
					// one coupon item only
					   $_citem_meta = explode( '|', $item[ 'coupon_items' ] );
					   $coupon_code =  array_shift( $_citem_meta );
                                           $coupon_code = substr($coupon_code, strpos($coupon_code, ":") + 1); 
                                           $description = array_shift( $_citem_meta );
                                           ...
                                           $coupon_item['discount_amount'] = $discount_amount;
