...
class OrderImpExpXML_OrderImport extends WP_Importer {
    ...
    public function import($import_type, $import_decision) {
        ...
        $this->log->add('order-xml-import', '---');
        $this->log->add('order-xml-import', __('Processing orders.', 'wf_order_import_export'));
        $merging = 1;
        $record_offset = 0;
        switch($import_type)
        {
        ...
        $this->log->add('order-xml-import', __('Finished processing Orders.', 'wf_order_import_export'));
	...
	$wp_memory = size_format( (WC()->version < '2.7.0')? woocommerce_let_to_num( WP_MEMORY_LIMIT ) : wc_let_to_num( WP_MEMORY_LIMIT ) );
        ...
        $this->log->add('order-xml-import', '---[ New Import ] PHP Memory: ' . $memory . ', WP Memory: ' . $wp_memory);
        $this->log->add('order-xml-import', __('Parsing order XML.', 'wf_order_import_export'));

        $this->parser = new OrderImpExpXML_Parser('shop_order');

        $this->parsed_data = $this->parser->parse_data($file);

        $this->log->add('order-xml-import', __('Finished parsing order XML.', 'wf_order_import_export'));
    ...
    private function process_orders($post,$import_type,$import_decision) {
        ...
        $this->add_import_result($view_status, __($out_updated_msg, 'wf_order_import_export'), $post['order_number'], $post['order_number'], $post['order_number']);
        $this->log->add('order-xml-import', sprintf(__('> &#8220;%s&#8221;' . $out_updated_msg, 'wf_order_import_export'), $post['order_number']), true);
        $this->log->add('order-xml-import', sprintf(__('> Finished importing order %s', 'wf_order_import_export'), $post['order_number']));
	...
	$settings[ 'ftp_server_path' ]	= $ftp_server_path;
        ...
        $local_file = WP_PLUGIN_DIR.'/order-import-export-xml/sample-files/temp-import.xml';
