...
class WF_OrderImpExpCsv_Exporter {
    ...
    public static function do_export($post_type = 'shop_order', $order_IDS = array(), $xmldata = '0') {
        ...
        if ($xmldata == '1') {
            ...
            if ($enable_ftp_ie) {
                ...
                $file = (!empty($settings['ord_auto_export_ftp_file_name']) ) ? $settings['ord_auto_export_ftp_file_name'] : $post_type . "-export-" . date('Y_m_d_H_i_s', current_time('timestamp')) . ".csv";
                $fp = fopen($file, 'w');
            } else {
            ...
            $row = apply_filters('hf_alter_csv_header', $row, $export_columns, $csv_columns); //Alter CSV Header
            ...
            if ($enable_ftp_ie) {
                ...
                $remote_file = ( substr($remote_path, -1) != '/' ) ? ( $remote_path . "/" . $file ) : ( $remote_path . $file );
    ...
    public static function get_orders_csv_row($order_id, $export_columns, $max_line_items, $user_columns_name = array()) {
        ...
        return apply_filters('hf_alter_csv_order_data', $order_data, $export_columns, $user_columns_name);
    }
