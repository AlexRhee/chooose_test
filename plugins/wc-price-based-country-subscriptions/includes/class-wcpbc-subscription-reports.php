...
class WCPBC_Subscription_Reports {
	...
	private static function add_exchange_rate_to_query( $query, $line_item_field, $search, $post_alias, $join_type ) {
		...
		$currency_rates = WCPBC_Admin_Report::get_currency_rates();

		if ( $currency_rates && ( $pos = strripos( $query, $search ) ) ) {

			$caseex = WCPBC_Admin_Report::caseex( $line_item_field, $currency_rates );									
			$query = substr( $query, 0, $pos) . ' ' . WCPBC_Admin_Report::join_order_currency( $post_alias, $join_type ) . ' ' . substr( $query, $pos );
