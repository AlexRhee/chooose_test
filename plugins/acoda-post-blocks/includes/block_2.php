...
*/
	...
	else if( $column_width == '1/1' && $page_layout == 'layout_one' )
	{
		$column = 'apb-col-4';
		$full_width = true;
		$featured_array = array('1','2','3');
	}
	else
	{
		$column = 'apb-col-6';
		$full_width = false;
		$featured_array = array('1','2');
	}
	...
	$output .=	'<div class="apb-module' . ( in_array( $count, $featured_array ) ? ' featured' : '' ) . ( $post->ID == $current_post_id && is_single() ? ' current-post-item' : '' ) .'">';

		if ( current_theme_supports('post-thumbnails') ) :

			$featured = ( in_array( $count, $featured_array ) ? true : false );
		...
		if ( ( in_array( $count, $featured_array ) || $excerpt == 'all' ) && $excerpt != 'disable' ) : 
